...
<div>
    <div>
    <form action="{{route('relatorios.pdf', $user->id)}}" method="GET">
        @csrf
        <button type="submit" class="btn btn-primary mt-2 mt-xl-0" style="float: right">Gerar PDF</button>
    </form>
    </div>
    <a href="{{route('relatorios.index')}}" class="btn btn-secondary mt-2 mr-2 mt-xl-0" style="float:right">Voltar</a>
</div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            // Obtenha o ID do elemento HTML
            var userId = $('#userId').data('user-id');


            // Crie um formulário oculto
            var form = $('<form>', {
                action: '/relatorios/pdf/' + userId,
                method: 'GET',
                target: '_blank' // Isso faz com que o link seja aberto em uma nova aba
            });
